...
import { InsertUser, users, prayerSlots, InsertPrayerSlot } from "../drizzle/schema";
...
export async function listPrayerSlots() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(prayerSlots).orderBy(prayerSlots.startMinutes);
}

export async function addPrayerSlot(slot: InsertPrayerSlot) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(prayerSlots).values(slot);
}
...
export async function getPrayerSlotByToken(token: string) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(prayerSlots).where(eq(prayerSlots.token, token)).limit(1);
  return result.length > 0 ? result[0] : undefined;
}
